...
export class WoActionsComponent implements OnInit, OnDestroy {
  ...
  private agentUrl = inject(API_AGENT_URL);
  ...
  loadActions() {
    this.http.get<any>(`${this.apiUrl}/api/config`).subscribe({
      next: (data) => {
  ...
  saveAction() {
    ...
    this.http.post(`${this.apiUrl}/save-actions`, { actions: this.actions }).subscribe({
      next: () => {
