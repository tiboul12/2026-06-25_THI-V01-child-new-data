...
export class WoHistoryComponent implements OnInit {
  ...
  async load() {
    ...
    this.error = '';
    try {
      const data: any = await this.http.get(`${this.apiUrl}/api/history`).toPromise();
  ...
  async toggleFeatured(entry: HistoryEntry, event: Event) {
    ...
    try {
      await this.http.put(`${this.apiUrl}/api/history/${entry.id}`, { featured: entry.featured }).toPromise();
    } catch (e) {
