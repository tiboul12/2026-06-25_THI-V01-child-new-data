...
export class TicketWidgetComponent implements OnDestroy {
  ...
  async loadTickets() {
    ...
    this.listError   = '';
    try {
      const data: any = await this.http.get(`${this.apiUrl}/api/tickets`).toPromise();
  ...
  listScreenshotUrl(ticket: WidgetTicket): string {
    return `${this.apiUrl}/api/tickets/${ticket.id}/screenshot`;
  }
  ...
  async saveEditTicket() {
    ...
    try {
      const updated: any = await this.http.put(
        `${this.apiUrl}/api/tickets/${this.editingTicket.id}`,
        this.editForm
      ).toPromise();
  ...
  async doDelete() {
    ...
    this.deleting = true;
    try {
      await this.http.delete(`${this.apiUrl}/api/tickets/${this.confirmDeleteId}`).toPromise();
  ...
  async loadComments(ticketId: string) {
    ...
    try {
      const data: any = await this.http.get(`${this.apiUrl}/api/tickets/${ticketId}/comments`).toPromise();
  ...
  async addComment() {
    ...
    try {
      const comment: any = await this.http.post(
        `${this.apiUrl}/api/tickets/${this.expandedTicketId}/comments`,
  ...
  async deleteComment(commentId: string) {
    try {
      await this.http.delete(`${this.apiUrl}/api/tickets/comments/${commentId}`).toPromise();
  ...
  async submit() {
    ...
    try {
      await this.http.post(`${this.apiUrl}/api/tickets`, payload).toPromise();
      this.close();
    } catch (e: any) {
