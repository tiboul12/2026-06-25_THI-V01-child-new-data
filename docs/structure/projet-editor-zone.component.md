...
- **Identifiant stable de section (`{{SID:folderId}}`)** : Chaque heading porte un marqueur stable dérivé du dossier (`composeHeading`), réinjecté à chaque reconstruction (`reconstructFromSections`). Le parsing (`parseContent`, `recomputeRanges`, `parseStructureNodes`) résout le `folderId` **prioritairement par SID** (puis chemin slugifié, puis nom). Conséquence : renommer un titre ou réordonner ne perd plus le lien section↔dossier et ne crée plus de dossier parasite. Visible (atténué) en mode Code, masqué en Structure/Édition. Migration automatique des projets legacy au chargement.
- **Création de titre atomique** : H1-H4 (barre de style mode Édition, slash-menu, et boutons « + » du mode Structure) ouvrent le popup partagé `worg-title-create-dialog`. À la validation, `createTitleSection` crée le dossier physique **d'abord** (`createFolder` → `folderId`), l'intègre dans `this.files`, puis insère le heading avec son SID. Remplace l'ancien `execCommand('formatBlock', H1-4)`.
...
5. **Création de titre par popup :** En mode Édition, sélectionner du texte → barre de style → H2 → le popup s'ouvre pré-rempli → valider → un dossier apparaît sur disque, le heading est inséré avec `{{SID}}` (masqué), la sidebar se met à jour. Vérifier qu'aucun dossier en double n'est créé.
6. **Stabilité au renommage/réordonnancement :** Renommer un titre puis recharger → le dossier est renommé (pas recréé), le SID conservé. Réorganiser plusieurs sections → aucun dossier parasite, aucun mauvais renommage (matching par SID, jamais par ordre).