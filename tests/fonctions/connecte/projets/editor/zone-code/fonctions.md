...
- **Vue board selon le mode** : le panneau `app-trello-board` est **masqué en mode Code** (edit) et **affiché en Structure/Preview** (condition `mode !== 'edit'`). `recomputeContentTrelloIds()` dérive les ids du marqueur dans la **section active** en mode Code, et de **tout le contenu** en Structure/Preview.
